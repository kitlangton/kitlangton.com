package zio.start

import com.raquo.laminar.api.L._

object Icons {
  def magnifier =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="20" height="21" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
<g opacity="0.6">
<path d="M14 14.5L10 10.5M11.3333 7.16667C11.3333 7.7795 11.2126 8.38634 10.9781 8.95252C10.7436 9.51871 10.3998 10.0332 9.9665 10.4665C9.53316 10.8998 9.01871 11.2436 8.45252 11.4781C7.88634 11.7126 7.2795 11.8333 6.66667 11.8333C6.05383 11.8333 5.447 11.7126 4.88081 11.4781C4.31462 11.2436 3.80018 10.8998 3.36683 10.4665C2.93349 10.0332 2.58975 9.51871 2.35523 8.95252C2.12071 8.38634 2 7.7795 2 7.16667C2 5.92899 2.49167 4.742 3.36683 3.86683C4.242 2.99167 5.42899 2.5 6.66667 2.5C7.90434 2.5 9.09133 2.99167 9.9665 3.86683C10.8417 4.742 11.3333 5.92899 11.3333 7.16667Z" stroke="#B4B4B4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</g>
</svg>
        """
      }
    )

  def plus =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
        <svg width="15" height="15" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M7 0C7.26522 0 7.51957 0.105357 7.70711 0.292893C7.89464 0.48043 8 0.734784 8 1V6H13C13.2652 6 13.5196 6.10536 13.7071 6.29289C13.8946 6.48043 14 6.73478 14 7C14 7.26522 13.8946 7.51957 13.7071 7.70711C13.5196 7.89464 13.2652 8 13 8H8V13C8 13.2652 7.89464 13.5196 7.70711 13.7071C7.51957 13.8946 7.26522 14 7 14C6.73478 14 6.48043 13.8946 6.29289 13.7071C6.10536 13.5196 6 13.2652 6 13V8H1C0.734784 8 0.48043 7.89464 0.292893 7.70711C0.105357 7.51957 0 7.26522 0 7C0 6.73478 0.105357 6.48043 0.292893 6.29289C0.48043 6.10536 0.734784 6 1 6H6V1C6 0.734784 6.10536 0.48043 6.29289 0.292893C6.48043 0.105357 6.73478 0 7 0Z"/>
        </svg>
        """
      }
    )

  def github =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M16.0453 0.70166C7.21062 0.70166 0.0452881 7.86566 0.0452881 16.7017C0.0452881 23.771 4.62929 29.7683 10.988 31.8843C11.7866 32.0323 12.0453 31.5363 12.0453 31.115V28.1363C7.59462 29.1043 6.66795 26.2483 6.66795 26.2483C5.93995 24.399 4.89062 23.907 4.89062 23.907C3.43862 22.9137 5.00129 22.935 5.00129 22.935C6.60795 23.047 7.45329 24.5843 7.45329 24.5843C8.87996 27.0297 11.196 26.323 12.1093 25.9137C12.252 24.8803 12.6666 24.1737 13.1253 23.775C9.57196 23.3683 5.83595 21.9963 5.83595 15.867C5.83595 14.119 6.46129 12.6923 7.48395 11.5723C7.31862 11.1683 6.77062 9.54033 7.63996 7.33766C7.63996 7.33766 8.98395 6.90833 12.0413 8.97766C13.3173 8.62299 14.6853 8.44566 16.0453 8.43899C17.4053 8.44566 18.7746 8.62299 20.0533 8.97766C23.108 6.90833 24.4493 7.33766 24.4493 7.33766C25.32 9.54166 24.772 11.1697 24.6066 11.5723C25.6333 12.6923 26.2533 14.1203 26.2533 15.867C26.2533 22.0123 22.5106 23.3657 18.948 23.7617C19.5213 24.2577 20.0453 25.231 20.0453 26.7243V31.115C20.0453 31.5403 20.3013 32.0403 21.1133 31.883C27.4666 29.7643 32.0453 23.7683 32.0453 16.7017C32.0453 7.86566 24.8813 0.70166 16.0453 0.70166Z" fill="#8B8B8B"/>
</svg>
        """
      }
    )

  def externalLink =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M10 6H6C5.46957 6 4.96086 6.21071 4.58579 6.58579C4.21071 6.96086 4 7.46957 4 8V18C4 18.5304 4.21071 19.0391 4.58579 19.4142C4.96086 19.7893 5.46957 20 6 20H16C16.5304 20 17.0391 19.7893 17.4142 19.4142C17.7893 19.0391 18 18.5304 18 18V14M14 4H20M20 4V10M20 4L10 14" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
        """
      }
    )

  def twitter =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="33" height="27" viewBox="0 0 33 27" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M32.0453 3.77764C30.868 4.30031 29.6026 4.65231 28.2746 4.81097C29.6306 3.99897 30.672 2.71231 31.1613 1.17897C29.8933 1.93097 28.488 2.47764 26.992 2.77231C25.796 1.49631 24.088 0.698975 22.2 0.698975C17.9613 0.698975 14.8466 4.65364 15.804 8.75897C10.3493 8.48564 5.51195 5.87231 2.27329 1.90031C0.553288 4.85097 1.38129 8.71097 4.30396 10.6656C3.22929 10.631 2.21595 10.3363 1.33195 9.84431C1.25995 12.8856 3.43995 15.731 6.59729 16.3643C5.67329 16.615 4.66129 16.6736 3.63195 16.4763C4.46662 19.0843 6.89062 20.9816 9.76529 21.035C7.00529 23.199 3.52795 24.1656 0.0452881 23.755C2.95062 25.6176 6.40262 26.7043 10.1093 26.7043C22.2986 26.7043 29.1853 16.4096 28.7693 7.17631C30.052 6.24964 31.1653 5.09364 32.0453 3.77764Z" fill="#8B8B8B"/>
</svg>
        """
      }
    )

  def remove =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M8 16C10.1217 16 12.1566 15.1571 13.6569 13.6569C15.1571 12.1566 16 10.1217 16 8C16 5.87827 15.1571 3.84344 13.6569 2.34315C12.1566 0.842855 10.1217 0 8 0C5.87827 0 3.84344 0.842855 2.34315 2.34315C0.842855 3.84344 0 5.87827 0 8C0 10.1217 0.842855 12.1566 2.34315 13.6569C3.84344 15.1571 5.87827 16 8 16V16ZM6.707 5.293C6.5184 5.11084 6.2658 5.01005 6.0036 5.01233C5.7414 5.0146 5.49059 5.11977 5.30518 5.30518C5.11977 5.49059 5.0146 5.7414 5.01233 6.0036C5.01005 6.2658 5.11084 6.5184 5.293 6.707L6.586 8L5.293 9.293C5.19749 9.38525 5.12131 9.49559 5.0689 9.6176C5.01649 9.7396 4.9889 9.87082 4.98775 10.0036C4.9866 10.1364 5.0119 10.2681 5.06218 10.391C5.11246 10.5139 5.18671 10.6255 5.2806 10.7194C5.3745 10.8133 5.48615 10.8875 5.60905 10.9378C5.73194 10.9881 5.86362 11.0134 5.9964 11.0123C6.12918 11.0111 6.2604 10.9835 6.3824 10.9311C6.50441 10.8787 6.61475 10.8025 6.707 10.707L8 9.414L9.293 10.707C9.4816 10.8892 9.7342 10.99 9.9964 10.9877C10.2586 10.9854 10.5094 10.8802 10.6948 10.6948C10.8802 10.5094 10.9854 10.2586 10.9877 9.9964C10.99 9.7342 10.8892 9.4816 10.707 9.293L9.414 8L10.707 6.707C10.8892 6.5184 10.99 6.2658 10.9877 6.0036C10.9854 5.7414 10.8802 5.49059 10.6948 5.30518C10.5094 5.11977 10.2586 5.0146 9.9964 5.01233C9.7342 5.01005 9.4816 5.11084 9.293 5.293L8 6.586L6.707 5.293Z" fill="#D84343"/>
</svg>
       """
      }
    )

  def add =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M8 16C10.1217 16 12.1566 15.1571 13.6569 13.6569C15.1571 12.1566 16 10.1217 16 8C16 5.87827 15.1571 3.84344 13.6569 2.34315C12.1566 0.842855 10.1217 0 8 0C5.87827 0 3.84344 0.842855 2.34315 2.34315C0.842855 3.84344 0 5.87827 0 8C0 10.1217 0.842855 12.1566 2.34315 13.6569C3.84344 15.1571 5.87827 16 8 16V16ZM9 5C9 4.73478 8.89464 4.48043 8.70711 4.29289C8.51957 4.10536 8.26522 4 8 4C7.73478 4 7.48043 4.10536 7.29289 4.29289C7.10536 4.48043 7 4.73478 7 5V7H5C4.73478 7 4.48043 7.10536 4.29289 7.29289C4.10536 7.48043 4 7.73478 4 8C4 8.26522 4.10536 8.51957 4.29289 8.70711C4.48043 8.89464 4.73478 9 5 9H7V11C7 11.2652 7.10536 11.5196 7.29289 11.7071C7.48043 11.8946 7.73478 12 8 12C8.26522 12 8.51957 11.8946 8.70711 11.7071C8.89464 11.5196 9 11.2652 9 11V9H11C11.2652 9 11.5196 8.89464 11.7071 8.70711C11.8946 8.51957 12 8.26522 12 8C12 7.73478 11.8946 7.48043 11.7071 7.29289C11.5196 7.10536 11.2652 7 11 7H9V5Z" />
</svg>
       """
      }
    )

  def cylinder =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M3 5.25V12.75C3 14.4075 5.6865 15.75 9 15.75C12.3135 15.75 15 14.4075 15 12.75V5.25M3 5.25C3 6.9075 5.6865 8.25 9 8.25C12.3135 8.25 15 6.9075 15 5.25M3 5.25C3 3.5925 5.6865 2.25 9 2.25C12.3135 2.25 15 3.5925 15 5.25M15 9C15 10.6575 12.3135 12 9 12C5.6865 12 3 10.6575 3 9" stroke="#9E9E9E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
       """
      }
    )

  def checkmarkFill =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="20" height="20" viewBox="0 0 20 20"  xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" clip-rule="evenodd" d="M10 18C12.1217 18 14.1566 17.1571 15.6569 15.6569C17.1571 14.1566 18 12.1217 18 10C18 7.87827 17.1571 5.84344 15.6569 4.34315C14.1566 2.84285 12.1217 2 10 2C7.87827 2 5.84344 2.84285 4.34315 4.34315C2.84285 5.84344 2 7.87827 2 10C2 12.1217 2.84285 14.1566 4.34315 15.6569C5.84344 17.1571 7.87827 18 10 18V18ZM13.707 8.707C13.8892 8.5184 13.99 8.2658 13.9877 8.0036C13.9854 7.7414 13.8802 7.49059 13.6948 7.30518C13.5094 7.11977 13.2586 7.0146 12.9964 7.01233C12.7342 7.01005 12.4816 7.11084 12.293 7.293L9 10.586L7.707 9.293C7.5184 9.11084 7.2658 9.01005 7.0036 9.01233C6.7414 9.0146 6.49059 9.11977 6.30518 9.30518C6.11977 9.49059 6.0146 9.7414 6.01233 10.0036C6.01005 10.2658 6.11084 10.5184 6.293 10.707L8.293 12.707C8.48053 12.8945 8.73484 12.9998 9 12.9998C9.26516 12.9998 9.51947 12.8945 9.707 12.707L13.707 8.707V8.707Z"/>
</svg>
      """
      }
    )

  def checkmark =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="2">
  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
</svg>      """
      }
    )

  def warning =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="2">
  <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
</svg>     """
      }
    )

  def warningSolid =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20">
  <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
</svg>
"""
      }
    )

  def exclamationSolid =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20">
  <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
</svg>
"""
      }
    )

  def shieldSolid =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20">
  <path fill-rule="evenodd" d="M10 1.944A11.954 11.954 0 012.166 5C2.056 5.649 2 6.319 2 7c0 5.225 3.34 9.67 8 11.317C14.66 16.67 18 12.225 18 7c0-.682-.057-1.35-.166-2.001A11.954 11.954 0 0110 1.944zM11 14a1 1 0 11-2 0 1 1 0 012 0zm0-7a1 1 0 10-2 0v3a1 1 0 102 0V7z" clip-rule="evenodd" />
</svg>
"""
      }
    )

  def clock =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2">
  <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
</svg>  """
      }
    )

  def star =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2">
  <path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
</svg>"""
      }
    )

  def chevronDownFill =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2">
  <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
</svg>"""
      }
    )

  def clockSolid =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20">
  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
</svg>  """
      }
    )

  def sadFaceFill =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20">
  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9a1 1 0 100-2 1 1 0 000 2zm7-1a1 1 0 11-2 0 1 1 0 012 0zm-7.536 5.879a1 1 0 001.415 0 3 3 0 014.242 0 1 1 0 001.415-1.415 5 5 0 00-7.072 0 1 1 0 000 1.415z" clip-rule="evenodd" />
</svg>
"""
      }
    )

  def flagFill =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20">
  <path fill-rule="evenodd" d="M3 6a3 3 0 013-3h10a1 1 0 01.8 1.6L14.25 8l2.55 3.4A1 1 0 0116 13H6a1 1 0 00-1 1v3a1 1 0 11-2 0V6z" clip-rule="evenodd" />
</svg>
"""
      }
    )

  def mailFill =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20">
  <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
  <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
</svg>
"""
      }
    )

  def folderDownload =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M4 4C3.46957 4 2.96086 4.21071 2.58579 4.58579C2.21071 4.96086 2 5.46957 2 6V14C2 14.5304 2.21071 15.0391 2.58579 15.4142C2.96086 15.7893 3.46957 16 4 16H16C16.5304 16 17.0391 15.7893 17.4142 15.4142C17.7893 15.0391 18 14.5304 18 14V8C18 7.46957 17.7893 6.96086 17.4142 6.58579C17.0391 6.21071 16.5304 6 16 6H11L9 4H4ZM11 9C11 8.73478 10.8946 8.48043 10.7071 8.29289C10.5196 8.10536 10.2652 8 10 8C9.73478 8 9.48043 8.10536 9.29289 8.29289C9.10536 8.48043 9 8.73478 9 9V10.586L8.707 10.293C8.61475 10.1975 8.50441 10.1213 8.3824 10.0689C8.2604 10.0165 8.12918 9.9889 7.9964 9.98775C7.86362 9.9866 7.73194 10.0119 7.60905 10.0622C7.48615 10.1125 7.3745 10.1867 7.2806 10.2806C7.18671 10.3745 7.11246 10.4861 7.06218 10.609C7.0119 10.7319 6.9866 10.8636 6.98775 10.9964C6.9889 11.1292 7.01649 11.2604 7.0689 11.3824C7.12131 11.5044 7.19749 11.6148 7.293 11.707L9.293 13.707L9.295 13.709C9.38758 13.8016 9.49749 13.875 9.61845 13.9251C9.73942 13.9752 9.86907 14.001 10 14.001C10.1309 14.001 10.2606 13.9752 10.3815 13.9251C10.5025 13.875 10.6124 13.8016 10.705 13.709L10.707 13.707L12.707 11.707C12.8892 11.5184 12.99 11.2658 12.9877 11.0036C12.9854 10.7414 12.8802 10.4906 12.6948 10.3052C12.5094 10.1198 12.2586 10.0146 11.9964 10.0123C11.7342 10.01 11.4816 10.1108 11.293 10.293L11 10.586V9V9Z"/>
</svg>
      """
      }
    )

  def desktop =
    div(
      onMountCallback { el =>
        el.thisNode.ref.innerHTML = """
<svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" fill="none">
<path d="M7.75 15L7 18L6 19H14L13 18L12.25 15M1 11H19M3 15H17C17.5304 15 18.0391 14.7893 18.4142 14.4142C18.7893 14.0391 19 13.5304 19 13V3C19 2.46957 18.7893 1.96086 18.4142 1.58579C18.0391 1.21071 17.5304 1 17 1H3C2.46957 1 1.96086 1.21071 1.58579 1.58579C1.21071 1.96086 1 2.46957 1 3V13C1 13.5304 1.21071 14.0391 1.58579 14.4142C1.96086 14.7893 2.46957 15 3 15Z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
      """
      }
    )
}
